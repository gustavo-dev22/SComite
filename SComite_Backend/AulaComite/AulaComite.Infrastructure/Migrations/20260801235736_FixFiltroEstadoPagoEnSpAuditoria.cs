...
{
    /// <inheritdoc />
    public partial class FixFiltroEstadoPagoEnSpAuditoria : Migration
    {
        /// <inheritdoc />
            ...
            migrationBuilder.Sql(@"
                ...
                BEGIN
                    ...
                    WITH IngresosCuotas AS (
                        ...
                        FROM Aulas a
                        LEFT JOIN Cuotas c ON c.AulaId = a.Id
                        -- 🚀 Se actualiza el filtro a 'COMPLETO', 'PAGADO' o 'APROBADO' y se contempla que el monto sea mayor a 0
                        LEFT JOIN CuotaDetalleEstudiante d ON d.CuotaId = c.Id 
                            AND (d.EstadoPago IN ('COMPLETO', 'PAGADO', 'APROBADO') OR d.MontoPagado > 0)
                        WHERE a.Estado = 1
                        GROUP BY a.Id
                    ),
